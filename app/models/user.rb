...
class User < ActiveRecord::Base
  ...
  validates_length_of :username, :within => 3..48
  validates_length_of :email, :within => 5..48
  validates_length_of :password, :within => 4..20, :if => :password_required?
  validates_length_of :profile, :maximum => 600
  ...
  def password_required?
    hashed_password.blank? || !self.password.blank?
  end
